<template>
  <article v-editable="blok">
    <header
      class="relative w-full h-screen flex items-end pb-80 -mt-32 plus-pattern"
    >
      <img
    ...
    <main
      class="relative z-50 container py-12 lg:flex lg:items-start bg-white bg-opacity-80 backdrop-blur-lg -mt-64 rounded-lg"
    >
      <section class="mb-12 lg:mb-0 lg:pr-32">
        <Headline v-if="blok.subheadline" size="small" class="mt-6 mb-10">{{
          blok.subheadline
        }}</Headline>
      ...
      <section class="sticky top-32 w-[300px] shrink-0 flex flex-col space-y-6">
        <div class="flex flex-col space-y-1.5 bg-white p-4 rounded-lg">
        ...
        <div class="flex flex-col space-y-3 bg-white p-4 rounded-lg">
